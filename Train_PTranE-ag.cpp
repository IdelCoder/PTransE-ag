...
private:
    ...
    void bfgs()
    {
        ...
        double margin = 1,margin_rel = 1, positive_margin = 0.05;
				...
				int e1 = rand_max(entity_num);
         		for (int k=0; k<batchsize; k++)
         		{
					...
					if (rand_tmp<25)
					{
                        ...
                        int count = 0;
                        while (ok[make_pair(e1,rel)].count(j) == 0 && count < 100)
                        {
                            j=rand_max(entity_num);
                            count ++;
                        }
                        if(ok[make_pair(e1,rel)].count(j)>0)
					...
					if (rand_tmp<50)
					{
                        ...
                        int count = 0;
                        while (ok[make_pair(j,rel)].count(e2)==0 && count < 100)
                        {
                            j=rand_max(entity_num);
                            count++;
                        }
                        if(ok[make_pair(j,rel)].count(e2)>0)
					...
					else
					{
                        ...
                        int count = 0;
                        while (ok[make_pair(e1,rel_neg)].count(e2) == 0 && count < 100)
                        {
                            rel_neg = rand_max(relation_num);
                            count++;
                        }
                        if (ok[make_pair(e1,rel_neg)].count(e2)>0)
                            train_kb(rel, rel_neg, false, positive_margin);
    ...
    // 比如，正例使用-1,而corrupted triplet 却是1，这是由求偏导的结果决定的。建议看一下Train_TransE.cpp的注释
