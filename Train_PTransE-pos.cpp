...
private:
         		...
         		for (int k=0; k<batchsize; k++)
         		{
					...
					if (rand_tmp<25)
					{
                        ...
                        if(candidate_size > 1) {
                            ...
                            train_pos(e1, e2, rel, e1, j, rel, margin);
                        }
					...
					if (rand_tmp<50)
					{
                        ...
                        if(candidate_size > 1) {
                            ...
                            train_pos(e1,e2,rel,j,e2,rel,margin);
                        }
    ...
    // (e1_a, e2_a, rel_a) 和 (e1_b, e2_b, rel_b) 都是有效三元组
    void train_pos(int e1_a,int e2_a,int rel_a,int e1_b,int e2_b,int rel_b,double margin) {
        double sum1 = calc_kb(e1_a, e2_a, rel_a);
        double sum2 = calc_kb(e1_b, e2_b, rel_b);
        if(sum1 - sum2 > margin) {
            res += sum1 - sum2 - margin;
            gradient_kb(e1_a, e2_a, rel_a, -1);
            gradient_kb(e1_b, e2_b, rel_b, 1);
        } else if(sum2 - sum1 > margin) {
            res += sum2 - sum1 - margin;
            gradient_kb(e1_a, e2_a, rel_a, 1);
            gradient_kb(e1_b, e2_b, rel_b, -1);
        }
    }
...
void prepare()
{
    ...
    FILE* f1 = fopen(("../" + dir + "/entity2id.txt").c_str(),"r");
	FILE* f2 = fopen(("../" + dir + "/relation2id.txt").c_str(),"r");
	int x;
    ...
    FILE* f_train = fopen(("../" + dir + "/train.txt").c_str(), "r");
    ...
    FILE* f_kb = fopen(("../" + dir + "/train_pra.txt").c_str(),"r");
	...
	FILE* f_confidence = fopen(("../" + dir + "/confidence.txt").c_str(),"r");
